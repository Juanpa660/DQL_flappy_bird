...
def createCNN():
    ...
    c_layer3 = convolutional_layer(input_x=c_layer2, shape=(3, 3, 64, 64), stride=1)
    #p_layer3 = max_pool_2by2(c_layer3)
    flat_layer = tf.reshape(c_layer3, [-1, 1600])
    flat_layer2 = tf.layers.dense(inputs=flat_layer, units=512, activation=tf.nn.relu,kernel_initializer=tf.random_normal_initializer(stddev=0.01))
...
q_ph = tf.placeholder(tf.float32, [None], 'q_value')
act_ph = tf.placeholder(tf.float32, [None, 2], 'actions')
...
def main(epsilon, explore_rate, gamma, learning_rate, obs_timesteps, batch, episodes, writer_name):
    ...
    """ COST FUNCTION """
    q_aprox = tf.reduce_sum(tf.multiply(q_out, act_ph), reduction_indices=1)
    q_loss = tf.reduce_mean(tf.square(q_ph - q_aprox))
    ...
    for episode in range(episodes):
        # REALIZA ACCION NO SALTAR
        act0 = [1, 0]
        img0, _, _ = game_state.frame_step(act0)
        # PROCESA IMAGEN
        ...
        # GUARDA LOS PRIMEROS 4 FRAMES
        obs_t = np.stack((img0_bin, img0_bin, img0_bin, img0_bin), axis=2)
        # VER SI MEJOR LE PASO LOS 4 FRAMES SOLO UNA VEZ COMO EN FLAPPY_MAIN
        """ EPISODIO """
        while True:
            # OBTENGO Q DE LA CNN
            ...
            # GUARDA ULTIMO FRAME A LOS 4 FRAMES
            ...
            cv2.imshow('image', img_pr)
            cv2.waitKey(0)
            cv2.destroyAllWindows()

            # ENTRENAR CUANDO T SEA MAYOR AL TIEMPO DE OBSERVACION
            if t > obs_timesteps:
                # OBTIENE UN BATCH RANDOM DENTRO DE INFO
                info_sample = random.sample(infos, batch)
                obs_batch, act_batch, rew_batch, obs_t2_batch, dones_batch = get_training_set(info_sample)

                # CALCULA EL Q OBSERVADO A PARTIR DEL BATCH
                q_batch_next = sess.run(q_out, feed_dict={obs_ph: obs_t2_batch})
                q_batch = []
                for i in range(0, len(dones_batch)):
                    if dones_batch[i]:
                        q_batch.append(rew_batch[i])
                    else:
                        q_batch.append(rew_batch[i] + gamma * np.max(q_batch_next[i]))
                ...
                if done:
                    ...
                    writer.add_summary(tf.Summary(value=[tf.Summary.Value(tag='Stats2/Q Loss',
                    simple_value=sess.run(q_loss,feed_dict={q_ph: q_batch, act_ph: act_batch, obs_ph: obs_batch}))]),episode)
            ...
            # PASA AL TIEMPO SIGUIENTE
            obs_t = obs_t2
            t += 1

        if episode == episodes - 1:
            saver.save(sess, 'saved_network/flappy-dqn2', global_step=episode)
    writer.close()
    sess.close()


if __name__ == "__main__":
    main(epsilon=0.1, explore_rate=1e-6, gamma=0.99, learning_rate=10e-7,
         obs_timesteps=10000, batch=32, episodes=30000, writer_name='./log/Flappy_new5')

    #main(epsilon=0.1, explore_rate=10e-7, gamma=0.99, learning_rate=10e-7,
    #     obs_timesteps=10000, batch=32, episodes=20000, writer_name='./log/Flappy_test')
